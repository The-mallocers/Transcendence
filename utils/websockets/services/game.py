...
class GameService(BaseServices):
    async def init(self, client: Clients) -> bool:
        ...
        if game_id:
            ...
            self.pL = await self.redis.json().get(RTables.JSON_GAME(self.game_key), Path(PlayerSide.LEFT))
            self.pR = await self.redis.json().get(RTables.JSON_GAME(self.game_key), Path(PlayerSide.RIGHT))
            return True
        else:
    ...
    async def _handle_start_game(self, data, client: Clients):
        status = GameStatus(await self.redis.json().get(RTables.JSON_GAME(self.game_key), Path('status')))
        if status_order.index(status) < status_order.index(GameStatus.STARTING):
            ...
            await self.redis.json().set(RTables.JSON_GAME(self.game_key), Path('status'), GameStatus.STARTING)
    ...
    async def _handle_paddle_move(self, data, client: Clients):
        ...
        if status is GameStatus.RUNNING:
            if str(client.id) == self.pL['id']:
                await self.redis.json().set(RTables.JSON_GAME(self.game_key), Path('player_left.paddle.move'), data['data']['args'])
            if str(client.id) == self.pR['id']:
                await self.redis.json().set(RTables.JSON_GAME(self.game_key), Path('player_right.paddle.move'), data['data']['args'])

    async def handle_disconnect(self, client):
        await self.redis.json().set(RTables.JSON_GAME(self.game_key), Path('status'), GameStatus.ENDING)
